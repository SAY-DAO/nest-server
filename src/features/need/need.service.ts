...
export class NeedService {
  ...
  async updateNeed(need: Need, request: Need): Promise<Need> {
      ...
      for (let k = 0; k < request.participants?.length; k++) {
        ...
        if (!user) {
          user = await this.userService.createUser({
            id_user: request.participants[k].id_user,
          });
        }
        ...
        if (!isSame && array1[0]) {
          const array = need.participants;
          const id_user = request.participants[k].id_user; //Element to be searched
  ...
  async createNeeds(request: NeedRequest): Promise<NeedEntity[]> {
    ...
    for (let i = 0; i < request.needData.length; i++) {
      ...
      if (thisNeed) {
        const updatedNeed = await this.updateNeed(
          thisNeed,
          request.needData[i],
        );
      ...
      for (let k = 0; k < request.needData[i]?.participants?.length; k++) {
        ...
        if (!user) {
          user = await this.userService.createUser({
            id_user: request.needData[i].participants[k].id_user,
          });
        }
        userList.push(user);
      }
      ...
      const need = await this.needRepository.save({
        ...
        expected_delivery_date:
          request.needData[i].expected_delivery_date &&
          new Date(request.needData[i]?.expected_delivery_date),
        ...
        ngo_delivery_date:
          request.needData[i].ngo_delivery_date &&
          new Date(request.needData[i]?.ngo_delivery_date),
        oncePurchased: request.needData[i].oncePurchased,
        paid: request.needData[i].paid,
        purchase_cost: request.needData[i].purchase_cost,
        purchase_date:
          request.needData[i].purchase_date &&
          new Date(request.needData[i]?.purchase_date),
        ...
        status_updated_at:
          request.needData[i].status_updated_at &&
          new Date(request.needData[i]?.status_updated_at),
        type: request.needData[i].type,
        type_name: request.needData[i].type_name,
        unavailable_from:
          request.needData[i].unavailable_from &&
          new Date(request.needData[i]?.unavailable_from),
        unconfirmed_at:
          request.needData[i].unconfirmed_at &&
          new Date(request.needData[i]?.unconfirmed_at),
        unpaid_cost: request.needData[i].unpaid_cost,
        unpayable: request.needData[i].unpayable,
        unpayable_from:
          request.needData[i].unpayable_from &&
          new Date(request.needData[i]?.unpayable_from),
        updated:
