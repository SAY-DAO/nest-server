...
export function fetchProductMessageContent(
  ...
  const productVoucher = {
    ...
    content:
      'با امضای دیجیتال این نیاز شما جزيیات این نیاز را تایید میکنید و همچنین امکان ذخیره غیر متمرکز و ثبت این نیاز بر روی بلاکچین را فراهم می‌کنید.  نیازی که دارای امضای دیجیتال مددکار، شاهد، میانجی و خانواده مجازی باشد نه تنها به شفافیت تراکنش‌ها کمک می‌کند، بلکه امکان تولید ارز دیجیتال (توکن / سهام) را به خویش‌آوندان می‌دهد تا SAY در جهت تبدیل شدن به مجموعه‌ای خودمختار و غیر متمرکز گام بردارد',
  } as const;
...
export function fetchProductMessageContent_legacy(
  need: NeedEntity,
  signerAddress: string,
  role: string,
) {
...
export function fetchServiceMessageContent_legacy(
  need: NeedEntity,
  signerAddress: string,
  role: string,
) {
